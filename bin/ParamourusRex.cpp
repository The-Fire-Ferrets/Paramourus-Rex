	...
	while(App.isOpen()) {
		...
		switch(state) {
			...
			case 1:	
				...
				break;
			case 2: 
				//DialogueView::update(&App, &state, elapsed_ms);
				//The bottom two lines should be implemented somewhere in DialogueView (and removed from Main) to switch to and start the level when dialogue is over
				LevelView::start();				
				state = 1;
				break;
			case 3: 
